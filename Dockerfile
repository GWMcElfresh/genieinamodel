...
RUN R -e "install.packages('tidyverse')" && \
 ...
 # R -e "remotes::install_github(repo = 'saezlab/MOFAcellulaR')" && \
  ...
  R -e "install.packages(c('FNN', 'igraph', 'future.apply', 'kernlab', 'forcats', 'progressr', 'future', 'twosamples', 'tidybayes', 'ggdist', 'luz', 'torch', 'data.table', 'marginaleffects', 'optparse'))" && \
# pip install mofapy2 && \
#  pip install scdef && \
  pip cache purge && \
  apt-get clean && \
